...
/// Availability metadata for the iOS-only `RecognizedItem`; it is not wrapped on macOS.
...
impl RecognizedItem {
    ...
    pub fn support_info() -> Result<AreaSupportInfo, VisionKitError> {
        ...
        let status = unsafe {
            ffi::recognized_item::vk_recognized_item_support_json(&raw mut support_json, &raw mut err_msg)
        };
