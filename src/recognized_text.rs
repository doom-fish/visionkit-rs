...
/// Availability metadata for the iOS-only `RecognizedItem.Text`; it is not wrapped on macOS.
...
impl RecognizedText {
    ...
    pub fn support_info() -> Result<AreaSupportInfo, VisionKitError> {
        ...
        let status = unsafe {
            ffi::recognized_text::vk_recognized_text_support_json(&mut support_json, &mut err_msg)
        };
